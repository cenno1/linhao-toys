...
export default function ProductGallery({ productName, items, videoSrc }: ProductGalleryProps) {
  ...
  const imageFit = active.src.includes("dumpling-advent-calendar-") || active.src.includes("pu-wrist-reflex-training-ball") || active.src.includes("custom-pu-high-rebound-ball") || active.src.includes("premium-custom-pu-character-figure") || active.src.includes("ultra-slow-rising-realistic-pu-food-squishy") ? "object-contain" : "object-cover";
  ...
  return (
    <div>
      ...
      {items.length > 1 && (
        <div className={items.length === 5 ? "mt-4 grid grid-cols-3 gap-3 sm:grid-cols-5" : "mt-4 grid grid-cols-4 gap-3"}>
